...
- **config**: configuration persistence and settings UI (`GenerationConfig`, `DbSeedSettingsState`,
...
Instead of bundling every JDBC driver up front, DBSeed4SQL lets the user choose the database engine from a configurable
list in
`src/main/resources/drivers.json`.

- If the selected driver is not installed, the plugin downloads it from **Maven Central** into `~/.dbseed-drivers/`
  after confirmation.
...
- For engines without deferred constraints, DBSeed4SQL performs a two-step strategy (`INSERT` + targeted `UPDATE`) to
  resolve FK cycles.
...
- Synthetic data generation with [DataFaker](https://www.datafaker.net/) respecting PKs, FKs, uniqueness, and
  nullability.
- Heuristics to recognize semantic column names like `email`, `name`, `uuid`, `title`, or `description`.
- Robust circular reference handling with deferred constraints (`SET CONSTRAINTS ALL DEFERRED`) or targeted post-insert
  updates.
